...
- **Remaining German-only service text translated to English:** `generate_web_io`'s Developer Tools > Actions description/field labels, and the persistent-notification text of `function_plan_connect`, `function_plan_sort`, `function_plan_stop`, `function_plan_activate` and `function_plan_visualize` (including the shared plan-activation status note) were still German-only after the service rename — now English throughout, consistent with every other user-facing string in the integration.
