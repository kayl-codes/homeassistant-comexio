...
- **Web-IO Range Guard:** A nightly check (04:00 window) verifies every analog Web-IO command's Min/Max against Comexio's live configuration and automatically corrects any drift. A new diagnostic `Web-IO Range Check` button lets you force the check on demand. Both paths post a summary notification of checked/fixed/failed commands.
