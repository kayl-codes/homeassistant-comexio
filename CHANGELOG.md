...
- **`function_plan_sort` could scramble a manually added comment/text block:** On a non-IO-cluster managed plan, a comment element other than the auto-generated managed-plan marker was treated as an ordinary orphan and repositioned into the sort grid like any unpaired marker/WebIO element. Comment/text blocks (type 14) are now excluded from the orphan list, matching the IO-cluster sort path, which already left them untouched.
