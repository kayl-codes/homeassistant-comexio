...
### ✨ New Features
- **Self-healing bus-load watchdog:** Detects a sustained rise in Comexio bus workload over a fixed six-hour window with a fixed 10-percentage-point threshold and cascades a stop/restart across the managed cluster function plans from last to first — the same manual recovery previously done by hand. This cascade-restart path is **on by default** (non-destructive, HA-managed plans only); a separate, default-**off** option can additionally trigger an unconditional emergency Comexio reboot if the five-minute average load exceeds 90%.
...
- **Function Plan Preview:** A live SVG diagram of a function plan, rendered directly in Home Assistant at the same layout Comexio Studio uses — same element positions, WebIO labels and wire routing. Wires turn red while their value is high (`1`), covering both real markers/IOs and block-internal outputs (e.g. an AND/OR gate or a timer with no marker of its own), and update live via a background poll. A stored backup snapshot can also be rendered fully offline. Ships as three entities (select/button/image) plus an optional interactive dashboard card (`comexio-plan-card`) with wildcard search, hover tooltips with net highlighting, remembered zoom level, and a debug box (live value log, direct value writes via `set_value`, command history/autocomplete, an exclude filter, pause/clear). See [FUNCTION_PLAN_PREVIEW.md](FUNCTION_PLAN_PREVIEW.md).
- **Function Plan Search:** `function_plan_search` finds which plans contain elements matching a text query — same wildcard syntax as the preview card's search bar — and auto-selects the plan when exactly one match is found.
- **Function Plan Analyze (experimental):** `function_plan_analyze` is a manual, read-only wiring health-check that flags likely mistakes (conflicting writers, unwired Set/Reset pins, dead outputs, suspicious double-wiring) and recognizes the "virtual button" self-reset idiom as a benign pattern instead of a false alarm.
- **Function Plan Flow Diagram (experimental):** `function_plan_flow_diagram` lays a plan out by signal-flow topology (input → logic → output) instead of its physical Comexio Studio position. Analyze and Flow Diagram share a new "Plan Analysis (experimental)" popup on the Plan Preview card, with a Findings/Flow-Diagram tab switch, wire hover-highlight (shared-net grouping including fan-out junction dots), and click-to-jump-to-element on a finding.
- **Function Plan Toggle button:** Starts/stops whichever plan is currently selected in the "Function Plans" entity, with a matching inactive-plan marker shown in the plan selector's dropdown.
- **Restore-as-copy:** `function_plan_restore` can restore into an independent new plan instead of overwriting the source, with an optional custom name and an auto-start opt-out. An in-progress notification and a restore lock prevent a second restore from racing the first, and comment/header blocks that would otherwise crash `run_fup` are recovered automatically.
- **Function Plan debris cleanup:** Marker/IO plan elements left behind after their Web-IO counterpart was removed (e.g. directly in Comexio Studio) are now detected and offered for cleanup through the existing sync button and repair-dialog flow.
- **Function Plan backup deletion:** `function_plan_delete_backups` removes one snapshot, all snapshots of a plan, or every stored backup of every plan; `function_plan_purge_orphaned_backups` removes only snapshots whose plan no longer exists in Comexio.
- **Preview auto-stop + extend:** An open live plan preview auto-stops its background polling after 15 minutes so a forgotten browser tab doesn't poll forever. A new `function_plan_preview_extend` service and the plan card's `/extend <n>` debug command extend a running preview session on demand.
- **Compact backup-restore card mode:** The Plan Preview card can be configured with only a `backup_entity` (no `entity`) to show a minimal card containing just the Restore trigger button, for placement next to the native backup selector.
- **Extension rename migration:** A Comexio-side extension rename (detected via the extension's stable serial number) is now migrated into Home Assistant automatically — affected entity `unique_id`s and the device identifier/name are rewritten in place at the next restart, preserving `entity_id` and statistics history instead of creating brand-new, orphaned entities.
...
- **More accurate pair-progress ETA, fewer notification updates:** The remaining-time estimate now uses the rate since the last update instead of the run-wide average since the sync started, so a plan's fixed setup cost (backup, stop, reload-wait) no longer skews early estimates high; the notification itself now only refreshes every 5th pair instead of every single one.
...
- **Preview session isolation:** The live plan preview's value poll now uses its own dedicated HTTP session with its own cookie jar/login, so a stalled Comexio response can no longer queue behind or block the main coordinator poll. All Comexio sessions now carry an explicit per-request timeout, and the preview poll backs off and disarms after 5 consecutive failures instead of retrying forever.
- **Connection-problem login retry:** A login failure caused by a connection problem (as opposed to bad credentials) now raises `ConfigEntryNotReady` so Home Assistant retries setup automatically, instead of being treated as an authentication error.
...
- **`ignored_markers` range parsing:** A range written with an `M` prefix on both ends (e.g. `M8-M12`) was rejected as invalid — only the first number's prefix was stripped before parsing. Both ends of a range now accept the optional prefix independently.
...
- **Function Plan preview mislabeled foreign Web-IO commands:** A plan element wired to a Web-IO command outside HA's own managed classes (e.g. a Comexio-Studio-authored device) showed a generic "WebIO ref=&lt;id&gt;" instead of its real name. A read-only label lexicon is now built across all Web-IO classes on the server during config parsing and preferred for preview rendering, matching Comexio Studio's own pill labels (`{deviceId}. {commandName}`).
- **`function_plan_sort` could scramble a manually added comment/text block:** On a non-IO-cluster managed plan, a comment element other than the auto-generated managed-plan marker was treated as an ordinary orphan and repositioned into the sort grid like any unpaired marker/Web-IO element. Comment/text blocks (type 14) are now excluded from the orphan list, matching the IO-cluster sort path, which already left them untouched.
- **Remaining German-only service text translated to English:** `generate_web_io`'s Developer Tools > Actions descriptions and field labels, and the persistent-notification text of `function_plan_connect`, `function_plan_sort`, `function_plan_stop`, `function_plan_activate` and `function_plan_visualize` (including the shared plan-activation status note) were still German-only after the service rename — now English throughout, consistent with every other user-facing string in the integration.
- **Options form input normalization:** `scan_interval` and the two Function Plan numeric options were converted to `int` outside the surrounding validation try/except, so a malformed value would raise an unhandled exception instead of a form error. The conversions now run inside their own dedicated try/except block, kept separate from the `ignored_markers` validation so a numeric-field error is never misattributed to it, and a missing `ignored_markers` field on submit now logs at `warning` instead of `debug`.
- **Function Plan auto-backup starvation:** A stalled Comexio response during the live preview's value poll could hang indefinitely (see preview session isolation above) and wedge the auto-backup cycle's lock forever, silently stopping all future auto-backups for the affected instance. An outer timeout around the whole backup cycle now guarantees the lock is always released, with a warning logged when a cycle can't start because the lock is still held.
- **Ghost firmware-update entities for offline extensions:** The `update.*` entity list and the orphan-cleanup active-ID set both built their extension list without checking the offline-extension filter, so an offline extension's firmware-update entity was recreated as a "ghost" device on every poll even when `include_offline_extensions` was off. Both paths now respect the same filter as every other entity platform.
- **Wire-hover state lost on live-poll reload:** Hovering a wire net on the Plan Preview card could stop being highlighted across a background SVG reload triggered by the live-value poll; the hover state now survives the reload.
